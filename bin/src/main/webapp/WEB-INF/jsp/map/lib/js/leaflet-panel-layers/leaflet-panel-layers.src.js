...
L.Control.PanelLayers = L.Control.Layers.extend({
	...
	updateLayers: function (group, name, layer) {
		for( var i = 0; i<this._layers.length; i++){
			var layerObj = this._layers[i];
			var layerName = layerObj.name;
			if(name == layerName){
//				this.removeLayer(this._layers[i].layer);	//아예 목록에서 사라져...
//				this._addLayer(overlays[i].layers[n], true, overlays[i].group, isCollapsed)
				layerObj.layer = layer;
				layerObj.id = layer._leaflet_id
			} 
		}
		
		//TODO 전체 그룹을 update하는데 꼭 그래야하는지 확인해보고 변경 가능하면 수정 : HH
		this._update();
	},
	...
	_getLayerByCode: function (code) {
		for (var i = 0; i < this._layers.length; i++) {
			if (this._layers[i] && this._layers[i].code == code) {
				return this._layers[i];
			}
		}
	},
	...
	_createItem: function (obj) {
		...
		//TODO 주제도 진입화면 표출 요소 지정, 나중에 그룹화하거나 config에서 불러올 것
	...
	_addItem: function (obj) {
		...
		if( obj.custom && obj.custom.layerType ){
			if( $(".leaflet-panel-layers-title > span:contains(\""+obj.name+"\")").length !== 0 ){
				console.log("해당 ITEM 존재 : ", obj);
				return
			}
		}
	...
	_onInputClick: function (e) {
		...
		for (i = 0; i < inputsLen; i++) {
			...
			if(e.currentTarget._layer.group.name == "AWS"){
				//TODO 현재는 그룹이 AWS만 존재해서 하드코딩. 나중에 config 쪽에서 확인하도록 수정 필요
				isDataLayer = true;
				obj = this._getLayerByCode(input._layer.code);
			}else{
				obj = this._getLayer(input.id);
			}
			
			//if(!isDataLayer){
				if (input.checked && !this._map.hasLayer(obj.layer)) {
					//해당 요소가 체크되어있고 이 맵에 레이어가 없으면
					
					//현재 레이어가 데이터가 있는 레이어인지 확인하고 아니면 새로 불러와서 넣어줘야해
					L.DomUtil.addClass(input.parentNode.parentNode, 'active');

					if(isDataLayer && !obj.isLayerExist && e.currentTarget._layer.code == obj.code){
					...
					//해당 요소가 체크되어있지 않고 이 맵에 레이어가 있으면
					L.DomUtil.removeClass(input.parentNode.parentNode, 'active');
					
					//TODO 해당 레이어를 꼭 삭제해야하는지, 다시 호출했을 때 조건이 같은 데이터라면 hide/show로 수정해도 될 것 같음 : HH
					this._map.removeLayer(obj.layer);
				} 
		}
		
		if(newObj != null){
			dataParam.name = newObj.name;
			dataParam.vars = newObj.code;
			dataParam.colorObs = newObj.code;
			dataParam.layerType = newObj.custom.layerType;
			dataParam.isLayerExist = true;
