...
L.Control.PanelLayers = L.Control.Layers.extend({
	...
	_createRadioElement: function (name, checked, obj) {
		...
		//console.log("_createRadioElement");
	...
	_createGroup: function (groupdata, isCollapsed) {
		...
		var self = this,
		groupdiv = L.DomUtil.create('div', this.className + '-group'), grouplabel, groupdiv_child, partdiv, groupslt, aicon, grouptit, groupexp;
		...
		groupdiv_child = L.DomUtil.create('div', this.className + '-groupdiv', groupdiv);
		partdiv = L.DomUtil.create('div', this.className + '-partdiv', groupdiv_child);
		groupslt = L.DomUtil.create('a', this.className + '-groupselector', partdiv);
		aicon = L.DomUtil.create('i', this.className + '-icon fas fa-eye-slash', groupslt);

		aicon.title = "전체 해제";

		L.DomEvent.on(partdiv, 'click', function (e) {

			var curtrg = e.target;
			var curlayersgroup = curtrg.parentNode.parentNode.parentNode.parentNode;
			var curviewtype = curtrg.classList.contains("show");

			for(var i=0; i<curlayersgroup.childElementCount; i++) {

				var curChild = curlayersgroup.childNodes[i];

				if(L.DomUtil.hasClass(curChild, 'leaflet-panel-layers-item')) {

					curbtntrg = curChild.childNodes[0].childNodes[0];

					if(!curviewtype && curbtntrg.checked) {
						curbtntrg.click();
					}
			...
			if(curviewtype) {
				curtrg.classList.remove("show");
				curtrg.classList.add("hide");
				curtrg.title = "전체 해제";
			}else {
				curtrg.classList.remove("hide");
				curtrg.classList.add("show");
				curtrg.title = "전체 선택";
			}
		}, this);
		...
		//div leaflet-panel-layers-groupselector
		//div leaflet-panel-layers-partselector
		//a	leaflet-panel-layers-selector
		//i	leaflet-panel-layers-icon
	...
	_initLayout: function () {
		...
		if (this.options.title) {
			var titlabel = L.DomUtil.create('label', this.className + '-title top');
			titlabel.innerHTML = '<span>' + this.options.title + '</span>';
			container.appendChild(titlabel);

			var topdiv = L.DomUtil.create('div', this.className + '-topdiv');
			var topslt = L.DomUtil.create('a', this.className + '-topselector', topdiv);
			var aicon = L.DomUtil.create('i', this.className + '-icon fas fa-chevron-up', topslt);

			topslt.href = "javascript:void(0);";
			topslt.title = "레이어 최소화";
...
L.control.panelLayers = function (baseLayers, overlays, options) {
		...
		//console.log("L.control.panelLayers");
