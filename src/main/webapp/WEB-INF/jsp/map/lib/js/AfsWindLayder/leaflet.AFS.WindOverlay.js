...
(function (factory, window) {
	...
	L.WindLayer = (L.Layer ? L.Layer : L.Class).extend({
		...
		_COLOR_INDEX_POSITION: null,
		_COLOR_INDEX_TO_TEXT: null,
		_UNITS: null,
		_ORIG_COORDS: [],
		_WIND_COORDS: [],
		_FIELD: {
		...
		/*-------------- PRIVATE ---------------*/
		_setInitOptions: function (options) {
			/**
			 * LEAFLET_AFS_CONFIG.OVERLAY.COORDS 확장
			 * https://lodash.com/
			 */
			this._ORIG_COORDS = _.clone(options.COORDS)
			this._WIND_COORDS = this._ORIG_COORDS.sort(function (a, b) {
				var d1 = a.y - b.y
				return d1 !== 0 ? d1 : a.x - b.x
			})
			this._FIELD = _.clone(options.FIELD)
			this._COLOR_INDEX_POSITION = options.COLOR_INDEX_POSITION
			this._COLOR_INDEX_TO_TEXT = options.COLOR_INDEX_TO_TEXT
		...
		_setForecastSize: function () {
			...
			var leftTop = this._map.project(L.latLng(coords[0].lat, coords[0].lng), this._ZOOM)
		...
		_drawColorIndex: function () {
			...
			drawColorIndex(this._colorIndexCanvas, 'VEC', this._colorScale, this._COLOR_INDEX_POSITION, this._COLOR_INDEX_TO_TEXT, this._UNITS)
		},
		...
		_convertData: function (data) {
			...
			var orig_coords = this._ORIG_COORDS
			var coords = this._WIND_COORDS
			var leftTop = orig_coords[0],
				rightBottom = orig_coords[orig_coords.length - 1]
			var size = this._FIELD
		...
		_startWindy: function () {
			...
			var zoom = this._map.getZoom()
			var ratio = this._forecastSize.x / this._FIELD.x
			var scale = this._map.getZoomScale(zoom, self._ZOOM) * ratio
			var leftTop = this._map.getPixelBounds().min.divideBy(scale)

			var latLngFnc = function (lat, lng) {
				return this._map.project(L.latLng(lat, lng), this._ZOOM)
			}.bind(self)
	...
	function drawColorIndex(canvas, dataType, colors, colorIndexPosition, colorIndexToText, mapUnits) {
		...
		if (['T3H', 'TMN', 'TMX', 'REH', 'WAV', 'WSD', 'VEC'].includes(dataType)) {
			styles.push({
				rgb: '#333333',
				bound: 1000
			})
		...
		var position = colorIndexPosition
		var colorToText = colorIndexToText
		var units = mapUnits
		...
		if (units[dataType]) {
			...
			context.font = '500 10px "Noto Sans KR", sans-serif'
			var text = units[dataType]
			if (
				localStorage &&
				localStorage.getItem('unit') &&
				(dataType === 'WSD' || dataType === 'VEC')
			) {
				var unit = localStorage.getItem('unit')
				text = Number(unit) === 0 ? 'm/s' : 'km/h'
			}
			context.fillText(text, 11, 8)
		}
	}
