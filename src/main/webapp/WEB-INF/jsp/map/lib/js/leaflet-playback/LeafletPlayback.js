...
L.Playback.Util = L.Class.extend({
  statics: {
    ...
    TimeStr: function(time) {
      ...
      return moment(new Date(time)).format("HH:mm:ss A");
    },
...
L.Playback.Clock = L.Class.extend({
  ...
  _tick: function (self) {
    if (self._cursor > self._trackController.getEndTime()) {
      ...
      //return;
      self._cursor = self._trackController.getStartTime();
    }
...
L.Playback.PlayControl = L.Control.extend({
    ...
    onAdd : function (map) {
        this._container = L.DomUtil.create('div', 'leaflet-control-layers leaflet-bar leaflet-control-layers-expanded');
        ...
        this._button = L.DomUtil.create('a', '', playControl);
        this._button.innerHTML = '<i class="fas fa-play"></i>';
        ...
        function play(){
            if (playback.isPlaying()) {
                playback.stop();
                self._button.innerHTML = '<i class="fas fa-play"></i>';
            }
            else {
                playback.start();
                self._button.innerHTML = '<i class="fas fa-stop"></i>';
            }                
        }
